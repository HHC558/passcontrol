...
class XlsxServiceTest {
    ...
    private fun sampleRecords() = listOf(
        AccountView(1, "GitHub", "user1", "p@ss&<word>", "user1@example.com", 1_700_000_000_000L),
        AccountView(2, "淘宝", "taobao_user", "abc123", null, 1_700_000_001_000L)
    )
    ...
    fun specialCharactersRoundTrip() {
        val bytes = service.writeRecords(
            listOf(AccountView(1, "A&B<C>", "u\"'", "p&<>&\"", null, 0L))
        )
        val rows = service.readRows(bytes)
